...
**Step2**: 点击菜单右侧的“+”按钮，将弹出“添加团队成员”对话框。 输入邀请的用户邮箱，默认是订阅号admin可以添加订阅号user权限的用户，然后点击“添加至订阅号”按钮将用户添加为订阅号成员。 
...
**Step2**: 在新建用户界面，填写用户邮箱和基本信息，带*的是必填项。在订阅号信息下面选择你要分配的订阅号并选择权限，点击“+”按钮在下面列表中添加一行订阅号的信息，可重复添加多条，要删除可点击列表后面的垃圾桶图标，该页面的订阅号信息列表，代表给该用户一次分配的多个或一个订阅号权限。
...
图11 - 订阅号列表
## 订阅号详情查看
在订阅号列表菜单可以点击展开某个订阅号后的操作按钮，选择详情可查看订阅号的详细信息，比列表中的基本信息多了可用点数，账户所有者，ID信息。
